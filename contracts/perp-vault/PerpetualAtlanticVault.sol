    ...
    ContractWhitelist
{
        ...
        returns (uint256 premium, uint256 tokenId)
    {
        ...
        _validate(
            requiredCollateral <=
                perpetualAtlanticVaultLp.totalAvailableCollateral(),
            3
        );
        ...
        assembly {
            let slot := sload(totalActiveOptions.slot)

            // Add the amount to the value stored in the slot
            slot := add(slot, amount)

            // Store the updated value back in the storage slot
            sstore(totalActiveOptions.slot, slot)
        }
    ...
    function calculateFunding(
        ...
        for (uint256 i = 0; i < strikes.length; i++) {
            ...
            _validate(
                latestFundingPerStrike[strikes[i]] !=
                    latestFundingPaymentPointer,
                5
            );
            ...
            uint256 amount = optionsPerStrike[strike] -
                fundingPaymentsAccountedForPerStrike[
                    latestFundingPaymentPointer
                ][strike];

            uint256 timeToExpiry = nextFundingPaymentTimestamp() -
                (genesis +
                    ((latestFundingPaymentPointer - 1) * fundingDuration));
        ...
        while (block.timestamp >= nextFundingPaymentTimestamp()) {
            if (lastUpdateTime < nextFundingPaymentTimestamp()) {
                ...
                collateralToken.safeTransfer(
                    addresses.perpetualAtlanticVaultLP,
                    (currentFundingRate *
                        (nextFundingPaymentTimestamp() - startTime)) / 1e18
                );
    ...
    function _updateFundingRate(uint256 amount) private {
        if (fundingRates[latestFundingPaymentPointer] == 0) {
            uint256 startTime;
            if (
                lastUpdateTime > nextFundingPaymentTimestamp() - fundingDuration
            ) {
